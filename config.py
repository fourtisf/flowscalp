...
RANGES: dict[str, dict[str, Any]] = {
    "risk_pct":             {"type": float, "min": 0.1, "max": 2.0,
                             "desc": "% equity risked per trade"},
    "sl_cap_pct":           {"type": float, "min": 0.2, "max": 1.0,
                             "desc": "max SL distance %; wider structural SL = skip trade"},
    "sl_buffer_pct":        {"type": float, "min": 0.02, "max": 0.15,
                             "desc": "SL buffer % beyond the sweep wick"},
    "tp_r":                 {"type": float, "min": 1.0, "max": 3.0,
                             "desc": "take profit in R multiples of the SL risk"},
    "time_stop_min":        {"type": int, "min": 5, "max": 60,
                             "desc": "flatten after N minutes if neither SL nor TP hit"},
    "leverage_cap":         {"type": int, "min": 1, "max": 10,
                             "desc": "max effective leverage (position is downsized)"},
    "daily_loss_limit_pct": {"type": float, "min": 1.0, "max": 10.0,
                             "desc": "realized day loss % that halts entries until 00:00 UTC"},
    "max_consec_losses":    {"type": int, "min": 2, "max": 10,
                             "desc": "losing streak that triggers a cooldown"},
    "cooldown_min":         {"type": int, "min": 30, "max": 480,
                             "desc": "cooldown length in minutes"},
    "atr_floor_bps":        {"type": float, "min": 2.0, "max": 50.0,
                             "desc": "skip entries when 1m ATR below this (dead tape)"},
    "atr_ceil_bps":         {"type": float, "min": 20.0, "max": 200.0,
                             "desc": "skip entries when 1m ATR above this (news chaos)"},
    "sweep_lookback":       {"type": int, "min": 10, "max": 120,
                             "desc": "1m bars defining the swing high/low to sweep"},
    "sweep_min_pen_pct":    {"type": float, "min": 0.01, "max": 0.2,
                             "desc": "min wick penetration % past the swept level"},
    "maker_timeout_s":      {"type": int, "min": 5, "max": 60,
                             "desc": "cancel unfilled post-only entry after N seconds"},
    "allow_taker_entry":    {"type": bool,
                             "desc": "IOC market fallback when the maker entry misses"},
    "sweep_vol_mult":       {"type": float, "min": 1.0, "max": 3.0,
                             "desc": "sweep bar volume must be >= this x SMA20; 1.0 = off"},
    "cvd_filter":           {"type": bool,
                             "desc": "require CVD absorption divergence on the sweep"},
    "session_windows":      {"type": "windows",
                             "desc": "UTC whitelist - new entries only inside these"},
    "blackout_windows":     {"type": "windows",
                             "desc": "UTC news windows - no new entries inside"},
    "funding_filter":       {"type": bool,
                             "desc": "skip the side that fights crowded funding"},
    "bias_filter":          {"type": bool,
                             "desc": "15m EMA200 trend filter (long above, short below)"},
}
