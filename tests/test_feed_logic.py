...
def feed():
    ...
    b = f.books["BTC"]
    b.delta_live_from = 1_000_000 // ONE_MIN_MS * ONE_MIN_MS  # trades stream "live"
    f.bk = b
    return f
...
async def test_candle_close_on_rollover_and_delta_alignment(feed):
    ...
    assert feed.queue.qsize() == 0 and len(feed.bk.candles_1m) == 0
    feed._on_candle(_candle_msg(t0 + ONE_MIN_MS))       # rollover → t0 bar closed
    kind, (coin, bar) = feed.queue.get_nowait()
    assert kind == "candle_1m_closed" and coin == "BTC"
    assert bar.ts == t0 and bar.close == 100_005.0
    assert list(feed.bk.deltas_1m) == [1.0]             # 1.5 buy − 0.5 sell
...
async def test_pre_stream_bars_get_none_delta(feed):
    ...
    feed._on_candle(_candle_msg(t0))
    assert list(feed.bk.deltas_1m) == [None]
...
async def test_duplicate_and_stale_candles_ignored(feed):
    ...
    assert len(feed.bk.candles_1m) == 1
    assert feed.bk.live_1m.ts == t0 + ONE_MIN_MS


async def test_finalizer_closes_dead_tape_bar(feed):
    t_old = (feed.bk.delta_live_from // ONE_MIN_MS - 100) * ONE_MIN_MS
    feed._on_candle(_candle_msg(t_old))                 # bar way in the past, no successor
    live = feed.bk.live_1m
    assert live is not None
    feed.bk.live_1m = None
    feed._finalize_1m("BTC", feed.bk, live)
    kind, (coin, bar) = feed.queue.get_nowait()
    assert kind == "candle_1m_closed" and bar.ts == t_old
    assert list(feed.bk.deltas_1m) == [None]            # before delta_live_from


async def test_mid_and_userfills_routing(feed):
    feed._on_mids({"mids": {"BTC": "100123.5", "ETH": "4000"}})
    kind, (coin, mid) = feed.queue.get_nowait()
    assert kind == "mid" and coin == "BTC" and mid == 100_123.5
    assert feed.mid == 100_123.5                        # legacy primary-coin view
    ...
    feed._on_user_fills({"fills": [{"coin": "BTC", "px": "100000", "sz": "0.1"},
                                   {"coin": "DOGE", "px": "0.4", "sz": "1"}]})
    kind, fill = feed.queue.get_nowait()
    assert kind == "fill" and fill["coin"] == "BTC"
    assert feed.queue.qsize() == 0                      # untracked coins dropped


async def test_multi_coin_books_are_isolated():
    class E:
        api_url = "x"
        coin = "BTC"
        coins = ("BTC", "SOL")

    f = Feed(E(), asyncio.Queue())
    for b in f.books.values():
        b.delta_live_from = 0
    t0 = 1_000_000 // ONE_MIN_MS * ONE_MIN_MS
    sol = dict(_candle_msg(t0), s="SOL", o="140", h="141", l="139", c="140.5")
    f._on_candle(sol)
    f._on_candle(dict(_candle_msg(t0 + ONE_MIN_MS), s="SOL"))
    kind, (coin, bar) = f.queue.get_nowait()
    assert coin == "SOL" and bar.close == 140.5
    assert len(f.books["BTC"].candles_1m) == 0          # BTC untouched
    f._on_mids({"mids": {"BTC": "62000", "SOL": "140.2"}})
    assert f.mids["SOL"] == 140.2 and f.mids["BTC"] == 62000.0
